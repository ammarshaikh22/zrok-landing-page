...
export default function Index() {
  ...
  return (
    <main className="bg-[#111111] text-white selection:bg-[#2563EB]">
      ...
      <section id="solutions" className="section"><div className="container-zrok"><SectionTitle eyebrow="The opportunity" title="Problems we solve" description="Helping businesses automate repetitive tasks, save time, and scale faster." /><div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">{problems.map(([title, text, Icon]) => <article key={title as string} className="dark-card p-6"><Icon size={20} className="text-[#2563EB]" /><h3 className="mt-9 text-base font-semibold">{title as string}</h3><p className="mt-3 text-sm leading-6 text-white/50">{text as string}</p></article>)}</div></div></section>
      ...
      <section id="contact" className="section"><div className="container-zrok grid gap-16 lg:grid-cols-12"><div className="lg:col-span-5"><SectionTitle eyebrow="Start a conversation" title="Build the operating system for your next stage of growth." description="Tell us where work feels slow, disconnected, or unnecessarily manual. We will help you see the path forward." /><div className="mt-10 space-y-5 text-sm text-white/60"><p>hello@zrok.ai</p><div className="flex gap-3 pt-3"><a href="#contact" className="social"><Linkedin size={17} /></a>
      {/* <a href="#contact" className="social"><Send size={16} /></a> */}
      </div></div></div><form className="grid gap-4 lg:col-span-7 sm:grid-cols-2" onSubmit={(e) => e.preventDefault()}>{[["Full Name", "text"],["Company Name", "text"],["Business Email", "email"],["Phone", "tel"],["Company Website", "url"]].map(([label, type]) => <label key={label} className="block text-sm text-white/60">{label}<input type={type} className="form-input mt-2" /></label>)}<label className="block text-sm text-white/60 sm:col-span-2">Message<textarea className="form-input mt-2 min-h-32 resize-y" /></label><button className="button-primary mt-2 justify-center sm:col-span-2" type="submit">Send inquiry <ArrowRight size={16} /></button></form></div></section>

      <footer className="border-t border-white/10 px-6 pb-8 pt-14 lg:px-10"><div className="mx-auto max-w-[1320px]"><div className="grid gap-10 grid-cols-2 lg:grid-cols-4"><div>  <a href="#home" className="flex items-center gap-2 text-xl font-bold tracking-[-0.06em]"><Image src="/footer.jpeg" alt="" width={30} height={50} className="rounded-md"/></a><p className="mt-4 max-w-48 text-sm leading-6 text-white/45">AI automation for modern businesses.</p></div>{[["Company", "About", "Case studies", "Contact"],["Services", "AI Agents", "Automations", "Integrations"],["Resources", "Insights", "FAQ", "Privacy"]].map(([heading, ...links]) => <div key={heading}><p className="text-sm font-medium">{heading}</p><div className="mt-4 space-y-3">{links.map((link) => <a key={link} href="#contact" className="block text-sm text-white/45 transition-colors hover:text-white">{link}</a>)}</div></div>)}</div><div className="mt-8 text-center  border-t border-white/10 pt-4 text-xs text-white/35"><p>© 2026 Zrok. All rights reserved.</p></div></div></footer>
    </main>
  );
}
